...
func (s *Story) Run(r io.Reader, w io.Writer, output func(string), c <-chan interface{}) (err error) {
	// frontend is the component that communicates with the tested backend
	frontend, _ := pgproto3.NewFrontend(r, w)
	...
	go func() {
		for {
			...
			b, err := frontend.Receive()
			if err != nil {
				errors <- err
				break
			}
			if closed || s.Filter == nil || s.Filter(b) {
				responseBuffer <- b
			} else if !closed {
				read <- 1
			}
